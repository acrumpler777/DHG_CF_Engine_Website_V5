    ...
    if (lists.length) {
      lists.forEach(function (el) {
        ...
        var updateListControls = function updateListControls() {
          listInfo.innerHTML = "".concat(list.i, " to ").concat(numberOfcurrentItems, " of ").concat(totalItem);
...
var handleNavbarVerticalCollapsed = function handleNavbarVerticalCollapsed() {
  ...
  var navbarVerticalCollapse = document.querySelector(Selector.NAVBAR_VERTOCAL_COLLAPSE);

  if (navbarVerticalToggle) {
    navbarVerticalToggle.addEventListener(Events.CLICK, function () {
      html.classList.toggle(ClassNames.NAVBAR_VERTOCAL_COLLAPSED); // Refresh Echarts

      var $echarts = document.querySelectorAll(Selector.ECHART_RESPONSIVE);
      !!$echarts.length && $echarts.forEach(function (item) {
  ...
  if (navbarVerticalCollapse) {
    navbarVerticalCollapse.addEventListener(Events.MOUSE_OVER, function () {
      if (utils.hasClass(html, ClassNames.NAVBAR_VERTOCAL_COLLAPSED)) {
        html.classList.add(ClassNames.NAVBAR_VERTOCAL_COLLAPSED_HOVER);
      }
    });
    navbarVerticalCollapse.addEventListener(Events.MOUSE_LEAVE, function () {
      if (utils.hasClass(html, ClassNames.NAVBAR_VERTOCAL_COLLAPSED_HOVER)) {
